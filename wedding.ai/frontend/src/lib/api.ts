...
export async function scoutChat(message: string, context: any) {
  const res = await fetch(`${API_BASE_URL}/agent/scout`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ message, context }),
  });
  if (!res.ok) throw new Error("Agentic recovery failed");
  return res.json();
}

export async function searchVenues(params: { city?: string; guests?: number; budget?: number }) {
  ...
  const res = await fetch(`${API_BASE_URL}/venues/search?${query.toString()}`);
...
export async function deleteSession(id: string) {
  ...
  if (!res.ok) throw new Error("Deletion array sync failed");
  return res.json();
}
