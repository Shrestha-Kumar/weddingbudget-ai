...
from models import (
    ...
    SundriesRequest, SessionCreate, SessionUpdate, LabelCreate, ScoutRequest
)
...
async def calculate_sundries(req: SundriesRequest):
    try:
        ...
        if req.hotel_tier:
            # Normalize: "5-Star Palace" -> "5star_palace"
            normalized = req.hotel_tier.lower().replace("-", "").replace(" ", "_")
            query = query.or_(f"hotel_tier.eq.{normalized},hotel_tier.is.null")
...
async def delete_session(token: str):
    try:
        res = supabase.table("budget_sessions").delete().eq("id", token).execute()
...
async def agent_scout(req: ScoutRequest):
    try:
        result = await agent.scout(req.message, req.context)
        return {"status": "success", "data": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/venues/search")
async def search_venues(city: str | None = None, guests: int = 0, budget: float = 0):
    try:
        query = supabase.table("venues").select("*").eq("is_active", True)
        if city:
        ...
        if budget > 0:
            venues = [v for v in venues if float(v["base_price_low"]) <= budget]
...
async def get_venue(venue_id: str):
    res = supabase.table("venues").select("*").eq("id", venue_id).execute()
    if res.data:
        return {"status": "success", "data": res.data[0]}
